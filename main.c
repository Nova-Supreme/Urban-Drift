...
int main()
{
    ...
    Texture2D mapbackground=LoadTexture("assets/map0.png");

    // World height stays fixed (tall driving area); world width is derived from the
    // map image's real aspect ratio so the map is scaled proportionally, not stretched.
    // This means the world is now wider than the screen, so the camera pans sideways too.
    float worldheight=3072.0f;
    float worldwidth=worldheight * ((float)mapbackground.width/(float)mapbackground.height);
    ...
    while(!WindowShouldClose()){
        ...
        // mapping mouse clicks (menu buttons) and for the final draw
        ...
        if (gamestate == STATE_PLAYING) {
            ...
            // --- AI CARS: move, despawn at border, periodic respawn, collision ---
            UpdateAICars(aicars, MAX_AICARS, worldwidth, worldheight, dt);
            PeriodicRespawnAICars(aicars, MAX_AICARS, worldwidth, worldheight, playerRadius, dt, &spawnTimer, SPAWN_INTERVAL);
            if (CheckAICarCollision(aicars, MAX_AICARS, pos, playerRadius)) {
                speed = 0.0f;
            ...
            if (IsResumeButtonClicked(resumeButton, GetMousePosition().x, GetMousePosition().y, viewX, viewY, scale)) {
        ...
        BeginTextureMode(target);
            ...
            DrawAICars(aicars, MAX_AICARS, cartextures, playervehicle.width, playervehicle.height);
            ...
            if (gamestate == STATE_PAUSED) {
                DrawPauseMenu(gamewidth, gameheight, resumeButton);
            }
